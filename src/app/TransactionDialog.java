		...
		saveBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				try {
					...
					if (date == null) {
						JOptionPane.showMessageDialog(rootPane, "Invalid \"Date\" field, try again.", "Error",
					...
					if (roundedValue > 10000000000000.00 || roundedValue < -10000000000000.00) {
						JOptionPane.showMessageDialog(rootPane, "\"Value\" field too big, try again.", "Error",
								JOptionPane.WARNING_MESSAGE);
						return;
					}
					...
					if (details.length() > 500) {
						JOptionPane.showMessageDialog(rootPane, "\"Details\" field too long, try again.", "Error",
								JOptionPane.WARNING_MESSAGE);
						return;
					}
					transaction = new Transaction(transaction.getId(), transaction.getUsername(), date, value, category,
							details);
					dispose();
				} catch (java.lang.NumberFormatException e) {
					JOptionPane.showMessageDialog(rootPane, "Invalid \"Value\" field, try again.", "Error",
							JOptionPane.WARNING_MESSAGE);
				} catch (java.lang.NullPointerException e) {
					JOptionPane.showMessageDialog(rootPane, "Missing or empty fields, try again.", "Error",
							JOptionPane.WARNING_MESSAGE);
				} catch (Exception e) {
					JOptionPane.showMessageDialog(rootPane, "Invalid fields, try again.", "Error",
							JOptionPane.WARNING_MESSAGE);
				}
			}
		});
